  ...
  render() {
    ...
    return (
      <footer className={styles.footer}>
        {
          categories
            .filter(category => // Filter out categories that have no users in it
              users
                ...
                <span className={styles.links}>
                  {
                    users
                      ...
                      .map(user => {
                        ...
                        if (mode === 'reddit') return (
                          <a key={username} href={`https://reddit.com/u/${username}`}>{`/u/${username}`}</a>
                        )
                      
                        if (mode === 'github') return (
                          <a key={username} href={`https://github.com/${username}`}>{username}</a>
                        )
